...
class MyMRSimulator:
    ...
    def reduceTask(self, kvs, namenode_fromR):
        ...
        tempKvs = {}
        for keyValue in kvs:
            if keyValue[0] not in tempKvs: tempKvs[keyValue[0]] = [keyValue[1]]
            else: tempKvs[keyValue[0]].append(keyValue[1])
        ...
        # [TODO]#
        for k, v in tempKvs.items():
            res = self.reduce(k=k, vs=v)
            if res is not None: namenode_fromR.append(res)
    ...
    def runSystem(self):
        ...
        for i in range(0, len(self.data), chunkSize):
            p = Process(target=self.mapTask, args=(self.data[i:i + chunkSize], namenode_m2r))
            p.start()
            processes.append(p)
        ...
        for i in range(len(namenode_m2r)):
            key, val, t_val = namenode_m2r[i][1][0], namenode_m2r[i][1][1], 0
            if str(type(key)) == 'int':
                for t in key: t_val += len(str(t))
            else:
                for char in str(key): t_val += ord(char)
            to_reduce_task[t_val % self.num_reduce_tasks].append([key, val])
